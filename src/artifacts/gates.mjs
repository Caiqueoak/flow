...
const KINDS = new Set(['command', 'builtin', 'agentic']);
...
export function parseGates(text, { source = 'gates.yaml' } = {}) {
  ...
  const gates = value.gates.map((gate, index) => {
    if (!gate || typeof gate !== 'object' || Array.isArray(gate)) throw new ArtifactValidationError(`gates[${index}] must be a mapping.`);
    if (typeof gate.id !== 'string' || !/^[a-z][a-z0-9-]*$/.test(gate.id)) throw new ArtifactValidationError(`gates[${index}].id must be lower kebab-case.`);
    if (ids.has(gate.id)) throw new ArtifactValidationError(`duplicate gate '${gate.id}'.`);
    ids.add(gate.id);
    if (!KINDS.has(gate.kind)) throw new ArtifactValidationError(`${gate.id}.kind must be command, builtin, or agentic.`);
    if (gate.kind === 'command' && (!gate.command || typeof gate.command !== 'string')) throw new ArtifactValidationError(`${gate.id} command gate requires command.`);
    if (gate.kind === 'agentic' && (!gate.profile || typeof gate.profile !== 'string')) throw new ArtifactValidationError(`${gate.id} agentic gate requires profile.`);
    if (gate.kind === 'agentic' && !AGENTIC_GATE_PROFILES[gate.profile]) throw new ArtifactValidationError(`${gate.id} references unknown agentic profile '${gate.profile}'.`);
