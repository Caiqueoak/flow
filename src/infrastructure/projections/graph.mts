import {
  ...
  topologicalOrder,
  type Backlog
} from '../../domain/work-item/backlog.mjs';
import type { DerivedBacklog } from './backlog.js';
...
export function generateGraphMarkdown(backlogText: string | Backlog | DerivedBacklog): string {
  const { work_items } = typeof backlogText === 'string' ? parseBacklog(backlogText) : backlogText;
  const items: GraphItem[] = work_items;
  const ordered = topologicalOrder(items);
  const statuses = new Map(items.map((item) => [item.id, item.state]));
  ...
  edges.forEach((edge, index) =>
    lines.push(`  linkStyle ${index} ${LINK_STYLES[statuses.get(edge.source) ?? 'eligible']};`)
  );
  lines.push('', ...CLASSES);
