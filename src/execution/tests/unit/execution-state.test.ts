...
import { emptyState, parseState, stringifyState, WORKFLOW_STEPS } from '../../execution-state.mjs';

type ExecutionState = {
  schema_version: number;
  execution: { phase: string; step: string };
  active: { work_item: string | null; task: string | null };
  stop_reason: string | null;
  migration: { status: string };
};
function state(overrides: Partial<ExecutionState> = {}) {
  return stringifyState({ ...emptyState(), ...overrides } as ExecutionState);
}

test('execution state round-trips canonical state', () => {
  const state = { ...emptyState(), active: { work_item: 'W101', task: 'W101-T001' } };
...
test('execution state accepts every workflow step, stop reason and migration status', () => {
  ...
  for (const stop_reason of [null, 'consequential_decision', 'external_action', 'unrecoverable_blocker', 'finished'])
    assert.equal(parseState(state({ stop_reason })).stop_reason, stop_reason);
  for (const status of ['not_required', 'pending_reconciliation', 'completed'])
