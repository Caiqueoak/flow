...
function history(root) {
  let output;
  try {
    output = execFileSync('git', ['log', 'HEAD', '--format=%H%x1f%ct%x1f%s%x1f%B%x1e'], {
      cwd: root,
      encoding: 'utf8',
...
function files(root, sha) {
  return execFileSync('git', ['show', '--format=', '--name-only', sha], { cwd: root, encoding: 'utf8' })
    .split(/\r?\n/)
    .filter(Boolean)
    .sort();
}
function valid(entry) {
  return (
    entry.work.length === 1 &&
    entry.task.length === 1 &&
    WORK_ITEM.test(entry.work[0]) &&
    TASK.test(entry.task[0]) &&
    entry.task[0].split('-T')[0] === entry.work[0]
  );
}
function context(root, id) {
  ...
  const work_item =
    parseBacklog(fs.readFileSync(backlogPath, 'utf8')).work_items.find((item) => item.id === id) ?? null;
  return !work_item
    ? { work_item, artifacts: null }
    : {
        work_item,
        artifacts: {
          backlog: '_flow/backlog.yaml',
          spec: `_flow/work-items/${work_item.folder}/spec.md`,
          tasks: `_flow/work-items/${work_item.folder}/tasks.yaml`
        }
      };
}
export function traceTasks(root, ids) {
  ...
  return new Map(
    ids.map((id) => {
      ...
      const status = invalid_commits.length
        ? 'invalid'
        : !commits.length
          ? 'missing'
          : commits.length > 1
      ...
      return [
        id,
        { task: id, commits, invalid_commits, status, ...(status === 'resolved' ? { commit: commits[0] } : {}) }
      ];
    })
  );
}
export function traceTask(root, id) {
  const result = traceTasks(root, [id]).get(id);
  const workId = id.split('-T')[0];
  const data = context(root, workId);
  let task_record = null;
  if (data.artifacts && fs.existsSync(path.join(root, data.artifacts.tasks)))
    task_record =
      parseTasks(fs.readFileSync(path.join(root, data.artifacts.tasks), 'utf8'), {
        expectedWorkItem: workId
      }).tasks.find((task) => task.id === `T${id.split('-T')[1]}`) ?? null;
  if (result.commit)
    result.commit = {
      ...result.commit,
      files: files(root, result.commit.sha),
      trailers: { 'Flow-Work-Item': workId, 'Flow-Task': id }
    };
...
export function traceWorkItem(root, id) {
  ...
  const commits = entries
    .filter((entry) => valid(entry) && entry.work[0] === id)
    .map((entry) => ({ ...entry, task: entry.task[0], files: files(root, entry.sha) }))
    .sort((a, b) => b.timestamp - a.timestamp || a.sha.localeCompare(b.sha));
...
export function runTrace({ args }) {
  ...
  if (!id) fail('flow trace requires W015-T003 or W015.');
  const root = projectRoot(args);
  const result = TASK.test(id) ? traceTask(root, id) : traceWorkItem(root, id);
  if (args.includes('--json')) return info(JSON.stringify(result, null, 2));
  if (TASK.test(id)) {
    if (result.status !== 'resolved') fail(`${id}: ${result.status} Flow trailer evidence.`);
    return info(
      `${id} -> ${result.commit.sha}\n${result.commit.subject}\nFiles:\n${result.commit.files.map((file) => `  ${file}`).join('\n')}`
    );
  }
  const listing = result.commits
    .map((entry) => `${entry.task} ${entry.sha} ${entry.subject}\n${entry.files.map((file) => `  ${file}`).join('\n')}`)
    .join('\n');
