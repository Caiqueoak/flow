...
import { configPath, readConfig } from '../shared/project-config.mjs';
...
function lockedPackageVersion(root, packageName) {
  ...
  try {
    return JSON.parse(fs.readFileSync(lockfile, 'utf8')).packages?.[`node_modules/${packageName}`]?.version || null;
  } catch {
    return null;
  }
}
...
function globalUpdatePlan(root, packageRoot, packageName) {
  try {
    const globalNodeModules = path.resolve(
      runNpm(['root', '--global'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim()
    );
    const globalPackageRoot = path.join(globalNodeModules, ...packageName.split('/'));
    if (
      path.resolve(packageRoot).startsWith(`${globalNodeModules}${path.sep}`) &&
      fs.existsSync(path.join(globalPackageRoot, 'package.json'))
    ) {
      return {
        npmArgs: ['update', '--global', packageName],
        cwd: root,
    ...
    // Fall through to local installation detection.
  }
  return null;
}

function updatePlan(root, packageRoot, packageName) {
  const global = globalUpdatePlan(root, packageRoot, packageName);
  if (global) return global;

  const nodeModules = containingNodeModules(packageRoot);
  if (nodeModules) {
    return { npmArgs: ['update', packageName], cwd: path.dirname(nodeModules), packageRoot, mode: 'local' };
  }

  const projectPackageRoot = packagePath(root, packageName);
  if (fs.existsSync(path.join(projectPackageRoot, 'package.json'))) {
    return { npmArgs: ['update', packageName], cwd: root, packageRoot: projectPackageRoot, mode: 'project' };
  }

  fail(
    `cannot determine how this Flow CLI was installed. Reinstall ${packageName} with npm, then run flow update again.`
  );
}

function repairDivergentProjectInstall(plan, packageName) {
  if (plan.mode === 'global') return;
  const expected = lockedPackageVersion(plan.cwd, packageName);
  ...
  try {
    ...
    if (installedPackageVersion(plan.packageRoot) !== expected)
      throw new Error(`npm install did not restore ${packageName}@${expected}.`);
    ...
    try {
      ...
      // Preserve the original npm error.
    }
    throw error;
  }
}

export function runUpdate({ args, packageRoot, packageName, version }) {
  const root = projectRoot(args);
  const config = readConfig(root, version);
  ...
  const plan = updatePlan(root, packageRoot, packageName);
  info(`Updating ${packageName} (${plan.mode} installation)...`);
  try {
    repairDivergentProjectInstall(plan, packageName);
    runNpm(plan.npmArgs, { cwd: plan.cwd, stdio: 'inherit' });
  } catch {
    fail('npm update failed. Existing project state and installed skills were not intentionally removed.');
  }

  if (!fs.existsSync(path.join(plan.packageRoot, 'package.json')))
    fail(`updated package not found at ${plan.packageRoot}.`);
  const latest = JSON.parse(fs.readFileSync(path.join(plan.packageRoot, 'package.json'), 'utf8'));
  for (const runtime of config.runtimes) {
    info(`✓ ${runtime.type}: ${path.relative(root, installRuntimeSkill(root, runtime, plan.packageRoot))}`);
  }
  const text = fs.readFileSync(configPath(root), 'utf8');
  fs.writeFileSync(
    configPath(root),
    text.replace(/(^framework:\s*\n(?:.*\n)*?\s+version:\s*)[^\n]+/m, `$1${latest.version}`),
    'utf8'
  );
