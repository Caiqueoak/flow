...
After that commit exists, run `flow task complete W###-T###`; this persists completion metadata as a separate administrative commit without Flow trailers. SHA evidence is always derived from the implementation trailers. On persistence failure, recover with `flow trace`/`flow doctor` rather than recreating implementation. Validate traceability, validate again and route onward.
